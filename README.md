...
- Vim対応CodeMirrorで事件記録と独立した時系列メモを編集・保存
- 事件記録・時系列メモ・人物別事件メモを共通タブから切り替え、種類をまたいで複数ペインに分割
- 15名それぞれに1つのVim対応事件メモを備えた、VS Code式プレビュータブと複数ペイン分割表示
- 顔アイコンや「魔法・証言・行動・証拠・アリバイ・矛盾・事実・推測」バッジを、クリックで現在行へ、ドラッグで任意の行へ付与するメモマーカー
- キャラクターの顔をMarkdown文中へドラッグし、編集画面とプレビューにインラインアイコンとして表示
- 魔法・トラウマ、アリバイ、証言の矛盾、確定事実をまとめて挿入できる事件メモテンプレート
